...
# ------------------------------------train------------------------------------
def train(net, train_loader, criterion, optimizer, epoch, device, log, efficient_net, res_net):
    # start train
    runningLoss = 0
    loss_count = 0
    ...
    for index, (imgs, labels) in enumerate(train_loader):
        ...
        with torch.no_grad():
            data = torch.cat((efficient_net(imgs),res_net(imgs)),1)
...
# ------------------------------------val------------------------------------
def val(net, val_loader, criterion, optimizer, epoch, device, log, val_len, train_start, efficient_net, res_net):
...
def main():
    ...
    efficient_net.load_state_dict(torch.load("effcientnet_model.pth"))

    res_net = torchvision.models.resnet50(pretrained=False)
    res_net.fc.out_features = 10
    res_net = res_net.to(device)
    res_net.load_state_dict(torch.load("resnet_model.pth"))
    ...
    for epoch in range(epochs):
        ...
        # start train
        train(net, train_loader, criterion, optimizer, epoch, device, log, efficient_net, res_net)

        # start val
        val(net, val_loader, criterion, optimizer, epoch, device, log, val_len, train_start, efficient_net, res_net)
